...
chrsnp_pval <- read.table("/home/bambrozi/2_CORTISOL/GWAS/BLUPF90/EXTREME_PHENO/keep_all_pheno/windows_10/chrsnp_pval", header = F)
colnames(chrsnp_pval) <- c("trait", "effect", "-log10(p-value)", "SNP", "Chromosome", "Position in bp")

merged_table <- merge(chrsnp_pval, chrsnpvar, by = "SNP")

merged_table <- merged_table[, c("trait", "effect", "SNP", "CHR", "BP", "Var", "-log10(p-value)")]

merged_table$p_value <- 10^(-merged_table$`-log10(p-value)`)

snp_map <- read.table("/home/bambrozi/2_CORTISOL/GWAS/BLUPF90/EXTREME_PHENO/keep_all_pheno/windows_10/snpmap.txt_clean", header = T)

var_pval <- merge(merged_table, snp_map, by.x = c("CHR", "BP"), by.y = c("CHR", "POS"), all.x = TRUE)

var_pval <- var_pval[, c("SNP_ID", "SNP", "CHR", "BP", "trait", "effect", "Var", "p_value", "-log10(p-value)")]

write.csv(var_pval, "/home/bambrozi/2_CORTISOL/GWAS/BLUPF90/EXTREME_PHENO/keep_all_pheno/windows_10/pval_for_first_snp_in_each_window.csv")
