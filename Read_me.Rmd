...
Now we are going to apply a corretion for multiple testing modifing the Bonferroni test adjusting not for the total number of SNPs but for the number of the independent segments in the genome.
...
gwas <- read.table(file = "/home/bambrozi/2_CORTISOL/GWAS/SNP_sig_Bonf.txt", 
                   head=T, stringsAsFactors = F)
...
write.table(out.qtl, file = "/home/bambrozi/2_CORTISOL/GALLO/out_qtl_60k.txt", 
            quote = FALSE, sep = "\t", row.names = FALSE, col.names = T)
...
```{r echo=FALSE}
# Read the CSV file into a data frame
data <- read.csv("C:/Users/galin/OneDrive - University of Guelph/0.Post doc/4_Cortisol/Git_Cortisol/out_qtl_50k.csv")
...
out.enrich_qtl <-qtl_enrich(qtl_db= qtl_UCD1_2, 
                       qtl_file= out.qtl, qtl_type = "Name",
                       enrich_type = "chromosome", chr.subset = NULL, 
                       padj = "fdr",nThreads = 2)

write.csv(out.enrich_qtl,"/home/bambrozi/2_CORTISOL/GALLO/out_enrich_qtl.csv")
 
```

Bellow we can realize that any QTL remain after the enrichment analysis.

```{r echo=FALSE}
# Read the CSV file into a data frame
data <- read.csv("C:/Users/galin/OneDrive - University of Guelph/0.Post doc/4_Cortisol/Git_Cortisol/out_enrich_qtl.csv")
...
#creating a GO list to use in CATEGORIZER.
library(tidyverse)
sig_enrich <- filter(result_enrich, adj_pvalue < 0.05)
write.table(sig_enrich, file = "/home/bambrozi/2_CORTISOL/GPROFILER/gene_enrich_sig.txt", row.names = FALSE, quote = F, col.names = FALSE)
write.csv(sig_enrich, file = "/home/bambrozi/2_CORTISOL/GPROFILER/gene_enrich_sig.csv")
```
...
```{r echo=FALSE}
# Read the CSV file into a data frame
data <- read.csv("C:/Users/galin/OneDrive - University of Guelph/0.Post doc/4_Cortisol/Git_Cortisol/gene_enrich_sig.csv")
...
category_counts <- table(sig_enrich$Category)
