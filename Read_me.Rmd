...
colnames(Merg_Cort_GEBVs)

library(tidyverse)

# Milk dataframe
milk <- select(Merg_Cort_GEBVs, ID, T4Cortisol, MILK)
sum(!is.na(milk$MILK))

# Protein dataframe
prot <- select(Merg_Cort_GEBVs, ID, T4Cortisol, PROT)
sum(!is.na(prot$PROT))

# Fat dataframe
fat <- select(Merg_Cort_GEBVs, ID, T4Cortisol, FAT)
sum(!is.na(fat$FAT))

# FE dataframe
fe <- select(Merg_Cort_GEBVs, ID, T4Cortisol, FE)
sum(!is.na(fe$FE))

# CK dataframe
ck <- select(Merg_Cort_GEBVs, ID, T4Cortisol, CK)
sum(!is.na(ck$CK))

# ME dataframe
me <- select(Merg_Cort_GEBVs, ID, T4Cortisol, ME)
sum(!is.na(me$ME))
...
columns <- c("MILK", "PROT", "FAT", "FE", "CK", "ME")
