...
- \( \beta_0 \), \( \beta_1 \), and \( \beta_2 \) are the intercept and regression coefficients, respectively.
- \( \epsilon \) represents the error term capturing unexplained variability.

The model can be implemented in R using the `lm` function, which fits linear models. The `BIRTH_YEAR` variable is converted to a factor to account for the categorical nature of birth years.
