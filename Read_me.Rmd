...
```{r echo=FALSE}
# Read the CSV file into a data frame
data <- read.csv("C:/Users/galin/OneDrive - University of Guelph/0.Post doc/4_Cortisol/Git_Cortisol/gene_enrich_15.csv")
...
<img src="image81.png" alt="My Image" style="display: block; margin-left: auto; margin-right: auto;">
...
GEBVs <- read.csv("/home/bambrozi/2_CORTISOL/GEBVs_Elora/ebvs_elora.csv")

#After talk to Lucas Alcântara, I got the meaning of some extra acronyms, so this is the full list of EBVs which we know the meaning.
GEBVs_to_use <- read.csv("/home/bambrozi/2_CORTISOL/GEBVs_Elora/ebv_names_lucas_06102024_BAG.csv")

# Check if the dataframes received from Samantha and Lucas Alcântara have the same animal IDs
# Sort the columns
sorted_cortisol_260_ID <- sort(cortisol_260$ID)
sorted_GEBVs_elora_id <- sort(GEBVs$elora_id)
identical(sorted_cortisol_260_ID, sorted_GEBVs_elora_id)

# Merging the dataframe with Cortisol (Samantha) values, with the dataframe with GEBVs values (Lucas Alcantara)
Merg_Cort_GEBVs <- merge(cortisol_260, GEBVs, by.x = "ID", by.y = "elora_id")


## Loop

# Define the columns of interest, that is the columns that we have meaning of the acronyms
# Create the vector with EBV values where Meaning is not empty
columns <- GEBVs_to_use %>%
  filter(Meaning != "") %>%
  pull(EBV)
...
# Loop through each column to create individual dataframes and save them as text files
for (col in columns) {
  # Create a dataframe for the current column
  df <- select(Merg_Cort_GEBVs, ID, T4Cortisol, all_of(col))
  
  # Store the dataframe in the list
  dataframes[[col]] <- df
  
  # Construct the filename
  filename <- paste0("df_", col, ".txt")
  
  # Construct the full file path
  full_path <- file.path("/home/bambrozi/2_CORTISOL/Correlation/df", filename)
  
  # Save the dataframe as a space-separated text file with full path
  write.table(df, file = full_path, sep = " ", row.names = FALSE, col.names = TRUE)
}
